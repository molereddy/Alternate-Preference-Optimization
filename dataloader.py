...
############ Imports for extending the class ############
import math
import time
...
############ End: Imports for extending the class ############
...
class CustomTrainerForgetting(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        # Used for storing the weights for loss, which could change dynamically during training
        self.loss_mask = {
            'forget_mask': True,
            'retain_mask': self.retain_wt!=0
        }
        self.weights_scheduler_args = hyperparams.get('weights_scheduler')

        super(CustomTrainerForgetting, self).__init__(*args, **kwargs)
    
    def weights_scheduler(self, epoch):
        if self.weights_scheduler_args is None:
            return
        mode = self.weights_scheduler_args.get('mode')
        if mode is None:
            return
        if mode == 'alternate_retain':
            forget_freq = self.weights_scheduler_args.get('forget_freq', 1)
            retain_freq = self.weights_scheduler_args.get('retain_freq', 1)
            total_freq = forget_freq + retain_freq
            
            if epoch%total_freq<forget_freq:
                self.loss_mask['forget_mask'] = True
                self.loss_mask['retain_mask'] = False
            else:
                self.loss_mask['forget_mask'] = False
                self.loss_mask['retain_mask'] = self.retain_wt!=0
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.loss_type == "grad_ascent":
            ...
            if self.loss_mask['forget_mask']:
                nll_forget, outputs = NLL(forget_inputs)
            else:
                nll_forget, outputs = torch.tensor(0.0).to(self.args.device), None
            loss_dict['NLL_forget'] += nll_forget 
            loss_dict['loss'] -= nll_forget
            ...
            loss_dict['NLL_forget'] += nll_forget
            loss_dict['loss'] -= nll_forget
            if self.loss_mask['retain_mask']:
                retain_loss = get_retain_loss(retain_inputs=retain_inputs)
            else:
                retain_loss = torch.tensor(0.0).to(self.args.device)
            loss_dict[f'{self.retain_type}_retain'] += retain_loss 
            loss_dict['loss'] += retain_loss
            ...
            if self.loss_mask['retain_mask']:
                ...
                kl_retain = torch.tensor(0.0).to(self.args.device) 
            loss_dict['KL_retain'] += kl_retain
            loss_dict['loss'] += kl_retain
            ...
            if self.loss_mask['forget_mask']:
                ...
                NLL_sub = torch.tensor(0.0).to(self.args.device)
            if self.loss_mask['retain_mask']:
                retain_loss = get_retain_loss(retain_inputs=retain_inputs)
            else:
                retain_loss =  torch.tensor(0.0).to(self.args.device)
            
            loss_dict['NLL_sub'] += NLL_sub
            loss_dict['loss'] += NLL_sub
            loss_dict[f'{self.retain_type}_retain'] += retain_loss
            loss_dict['loss'] += retain_loss
            ...
            if self.loss_mask['forget_mask']:  
                ...
                nll_forget, outputs = torch.tensor(0.0).to(self.args.device), None
                nll_sub = torch.tensor(0.0).to(self.args.device)
            loss_dict['NLL_forget'] += self.alpha * nll_forget 
            loss_dict['loss'] -= self.alpha * nll_forget
            loss_dict['NLL_sub'] += nll_sub  
            loss_dict['loss'] += nll_sub
            if self.loss_mask['retain_mask']:
                retain_loss = get_retain_loss(retain_inputs=retain_inputs)
            ...
            loss_dict['DPO_sub_forget'] += dpo_sub_forget 
            loss_dict['loss'] += dpo_sub_forget
            retain_loss = get_retain_loss(retain_inputs=retain_inputs)
            loss_dict[f'{self.retain_type}_retain'] += retain_loss
            loss_dict['loss'] += retain_loss  
            ...
            loss_dict['PO_forget'] += po_forget 
            loss_dict['loss'] += po_forget
            if self.loss_type == 'subnpo':
                ...
                loss_dict['PO_sub'] += po_sub
                loss_dict['loss'] += po_sub
            if self.loss_mask['retain_mask']:   
                retain_loss = get_retain_loss(retain_inputs=retain_inputs)
            else:
                retain_loss = torch.tensor(0.0).to(self.args.device) 
            ...
            if self.loss_mask['retain_mask']:   
                ...
                retain_loss = torch.tensor(0.0).to(self.args.device) 
            loss_dict['{self.retain_type}_retain'] += retain_loss
            loss_dict['loss'] += retain_loss
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        for k, v in loss_dict.items():
            loss_dict[k] = v.detach()/ self.args.gradient_accumulation_steps
    ...
    def _inner_training_loop(
        ...
        # Vineeth Change start
        tr_loss_dict = defaultdict(float)
        # Vineeth Change end
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ## Vineeth Change start
            self.weights_scheduler(epoch)
            logger.info(self.loss_mask)
            ## Vineeth change end here
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    ...
                    tr_loss_step = tr_loss_step_dict['loss']
                    # Vineeth Change end
                if (
                    ...
                    # Vineeth Change start here
                    for k, v in tr_loss_step_dict.items():
                        tr_loss_dict[k] += v 
                    # Vineeth change end here
    ...
    def _maybe_log_save_evaluate(self, tr_loss, tr_loss_dict, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            # Vineeth Change start
            tr_loss_scalar_dict = {k:self._nested_gather(v).mean().item() for k, v in tr_loss_dict.items()}
            # Vineeth Change end

            # reset tr_loss to zero
            tr_loss -= tr_loss
            # Vineeth Change start
            for k, v in tr_loss_dict.items():
                tr_loss_dict[k] = v-v
            # Vineeth Change end
