from data_module import TextForgetDatasetQA, TextForgetDatasetSubFullQA, TextForgetDatasetIDKFullQA
...
# from gib_eval import add_gibberish_evals
from utils import get_model_identifiers_from_yaml, get_model_utility_v2, rearrange_cols
# from agg_results.plot_traj import plot_trajectory
# from agg_results.plot_hist import plot_histograms
...
def main(cfg):
    ...
    model = AutoModelForCausalLM.from_pretrained(model_cfg["ft_model_path"], **model_kwargs)
    ...
    # once dataset collected DPO loss wors the same way for all methods
    if 'dpo' in cfg.forget_loss: # idkdpo/subdpo etc.
    ...
    batch_size = cfg.batch_size # usually 5
    gradient_accumulation_steps = 35 // cfg.batch_size # effective bsz always 35
    ...
    warmup_steps, logging_steps = max(1, steps_per_epoch), max(1, max_steps//20)
    ...
    print(f"gradient_accumulation_steps: {gradient_accumulation_steps}", 
          ...
          f"warmup_steps: {warmup_steps}", f"logging_steps: {logging_steps}")
